...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
  ...
  if ($display_all) {
    $stmt = $conn->prepare("SELECT * FROM deal ORDER BY to_date ASC");
    $stmt->execute();
    $result = $stmt->get_result();
    $result_deals = $result->fetch_all(MYSQLI_ASSOC);
  } else {
    // Select available deals
    $stmt = $conn->prepare("SELECT * FROM deal WHERE NOT (? <= from_date OR ? >= to_date)");
  ...
  $stmt = $conn->prepare("INSERT INTO `deal`(`destination`, `from_date`, `to_date`, `price_per_day`) VALUES (?,?,?,?)");
  ...
  $stmt = $conn->prepare("UPDATE deal SET destination=?, from_date=?, to_date=?, price_per_day=? WHERE id=?");
